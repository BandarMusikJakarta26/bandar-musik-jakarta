    ...
    return registerFields.map((field, index)=>{
        return (
            <div className="input-field relative" key={index}>
                ...
                <input {...register(field.name === "username" ? "username" : field.name === "email" ? "email" : "password")} type={field.type} name={field.name} placeholder={field.placeholder} className=" w-full md:text-[20px] py-3 px-4 bg-primary border-b-2 border-third"/>
