...
const TabPencarian = function({ result, keyword }: { result: { products: any[], brands: any[], categories: any[] }, keyword: string | any }){
    return (
        <div className={`${result.products.length == 0 && result.brands.length == 0 && result.categories.length == 0 ? 'h-0 hidden' : false} pencarian absolute bg-primary top-[10px] text-[14px] w-full h-[300px] overflow-y-scroll`}>
            { result.categories.length > 0 ? <SearchCategories categories={result.categories} keyword={keyword}/> : false}
