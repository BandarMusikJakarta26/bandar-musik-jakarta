...
export default async function addProductService(req, res){
    const { name, harga, diskon, deskripsi, brand, kategori, tokped } = req.body
    let filesName = []
    for(let i=0; i<req.files.length;i++){ filesName.push(req.files[i].filename) }
    const produk = await db.produk.create({ data: { name, harga, deskripsi, tokopedia: tokped, images: filesName, categoryName: kategori, brandName: brand, diskon } })
